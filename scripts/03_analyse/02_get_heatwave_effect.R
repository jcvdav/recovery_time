...
#Load data
MHWs <- read.csv(here::here("data", "Final_Data_MPAs_combined_thresold_none.csv"), stringsAsFactors = TRUE) %>% #**** Let's make strings be factors rather than character
...
effects <- diff_Abalone %>% 
  mutate(species = "Haliotis spp") %>% 
  rbind(mutate(diff_Sea_Cucumber, species = "Parastichopus parvimensis")) %>% 
  rbind(mutate(diff_SeaUrch, species = "Sea urchins")) %>% 
  group_by(species) %>% 
  summarize(MHW_effect =  -1 * mean(MHW) / 100)

saveRDS(effects, here::here("data", "MHW_effects_on_inverts.rds"))
